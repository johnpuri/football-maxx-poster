...
async function main(){
  console.log("=== FIXED 3 reels — single source of truth from ytTitle ===");
  ...
  while(results.length<3 && attempts<30){
    ...
    if(!found) console.log(`  No valid candidate for pick ${pick.query} — next attempt`);
    await new Promise(r=>setTimeout(r,2000));
  }
  fs.writeFileSync("/tmp/fixed_3_results.json", JSON.stringify(results,null,2));
  console.log("\n=== FIXED 3 DONE ===");
  console.log(JSON.stringify(results,null,2));
  if(results.length<3) console.log(`WARNING only ${results.length}/3 produced`);
