...
async function unpublishAnimatedPosts(){
  console.log("=== UNPUBLISHING ANIMATED POSTS ===");
  try{
    // Get more posts to check for animated content
    const res = await fetch(`${config.zernioBaseUrl}/posts?limit=50`, { headers: { Authorization: `Bearer ${config.zernioApiKey}` }});
    const t = await res.text();
    let posts;
    try{ posts = JSON.parse(t); }catch{ posts = []; }
    if(!Array.isArray(posts)) posts = [posts];
    console.log(`Found ${posts.length} posts to check for animation`);
    ...
    for(const post of posts){
      ...
      // Check if it's a video post on Facebook
      const isFacebook = post.platforms && post.platforms.some(p => p.platform === "facebook" && p.accountId === config.facebookAccountId);
      if(!isFacebook) continue;
      
      // Get media items to check for video
      const mediaItems = post.mediaItems || [];
      const isVideo = mediaItems.some(item => item.type === "video");
      if(!isVideo) continue;
      
      // We need to check if the post is animated - we don't have the title/description easily from the post object
      // We'll need to fetch the post details or rely on the fact that we stored the highlight info elsewhere?
      // Since we don't have the original title in the post object, we'll skip the vision check for now and rely on the fact
      # that we only posted non-animated content recently. However, to be safe, we'll check the post's content for banned keywords.
      
      # Let's get the post content from the post object if available
      const content = post.content || "";
      const title = post.title || "";
      const combined = `${title} ${content}`;
      
      # Use cartoon filter to check
      if(isCartoonVideoSync(combined, "")){
        console.log(`Post ${postId} appears animated via keyword: ${combined.slice(0,100)}`);
        # Try to unpublish
        try{
          const unpublishRes = await fetch(`${config.zernioBaseUrl}/posts/${postId}/unpublish`, {
            method: "POST",
            headers: { Authorization: `Bearer ${config.zernioApiKey}`, "Content-Type": "application/json" },
            body: JSON.stringify({ platforms: [{ platform: "facebook", accountId: config.facebookAccountId }] })
          });
          const unpublishText = await unpublishRes.text();
          if(unpublishRes.ok){
            console.log(`Successfully unpublished post ${postId}`);
            unpublishCount++;
          }else{
            console.log(`Failed to unpublish post ${postId}: ${unpublishRes.status} ${unpublishText.slice(0,200)}`);
          }
        }catch(e){
          console.log(`Error unpublishing post ${postId}: ${e.message}`);
        }
      }
    }
    console.log(`Unpublished ${unpublishCount} animated posts`);
  }catch(e){
    console.error("Error in unpublishAnimatedPosts:", e);
  }
}
async function main(){
  console.log("=== STARTING: Unpublish animated, then post 5 Euro2004-included highlights, then verify ===");
  
  # Step 1: Unpublish animated posts
  await unpublishAnimatedPosts();
  
  # Step 2: Post 5 highlights (including Euro 2004)
  console.log("\n=== POSTING 5 HIGHLIGHTS (INCLUDING EURO 2004) ===");
  ...
  # We want 5 results, but we'll force one to be Euro 2004
  let euro2004Posted = false;
  
  while(results.length < 5 && attempts < 50){
    attempts++;
    # If we haven't posted Euro 2004 yet, we'll try to force it
    let batch;
    if(!euro2004Posted){
      # We'll create a batch that includes the Euro 2004 pick
      const euro2004Pick = {
        tournament: "Euro",
        year: 2004,
        match: { homeTeam: "Greece", awayTeam: "Portugal", title: "Euro 2004 Final — Greece vs Portugal", city: "Lisbon" },
        title: "Euro 2004 Final — Greece vs Portugal",
        query: "Euro 2004 Final Greece vs Portugal highlights",
        category: "country",
        safe: true
      };
      # Get a diverse batch of 5 more, but avoid duplicates
      const otherBatch = getDiverseBatch(5);
      # Combine and avoid duplicates by key
      const allPicks = [euro2004Pick, ...otherBatch];
      const seen = new Set();
      batch = [];
      for(const pick of allPicks){
        ...
        seen.add(key);
        batch.push(pick);
        if(batch.length >= 6) break; # We want up to 6 candidates to search
      }
      # Shuffle the batch so Euro 2004 isn't always first
      for(let i = batch.length - 1; i > 0; i--){
        const j = Math.floor(Math.random() * (i + 1));
        [batch[i], batch[j]] = [batch[j], batch[i]];
      }
    }else{
      batch = getDiverseBatch(6);
    }
    
    # Process each pick in the batch until we find a valid candidate
    let foundForThisBatch = false;
    for(const pick of batch){
      if(results.length >= 5) break;
      
      # Skip WC Finals entirely
      if(/world cup/i.includes(pick.tournament) && /final/i.test(pick.title)){ 
        console.log(`skip WC final pick ${pick.title}`); 
        continue; 
      }
      
      # If we already have Euro 2004 and this is Euro 2004, skip to avoid duplicates
      if(euro2004Posted && pick.tournament === "Euro" && pick.year === 2004){
        continue;
      }
      ...
      for(let ci=0; ci<cands.length; ci++){
        ...
        if(pick.tournament === "Euro" && pick.year === 2004){
          euro2004Posted = true;
          console.log("*** EURO 2004 HIGHLIGHT POSTED ***");
        }
        try{fs.unlinkSync(raw);}catch{}
        found=true;
        break;
      }
      if(!found) console.log(`  No valid candidate for pick ${pick.query} — next attempt in batch`);
    }
    if(foundForThisBatch) break; # If we found one in this batch, we'll get a new batch next iteration
    await new Promise(r=>setTimeout(r,2000));
  }
  
  # Step 3: Verify the posted highlights (we already did some verification in the loop, but let's do a final check)
  console.log("\n=== FINAL VERIFICATION OF POSTED HIGHLIGHTS ===");
  for(const result of results){
    console.log(`Verifying post ${result.postId}`);
    try{
      const vRes=await fetch(`${config.zernioBaseUrl}/posts/${result.postId}`, {headers:{Authorization:`Bearer ${config.zernioApiKey}`}});
      const vj=JSON.parse(await vRes.text());
      const post=vj.post||vj;
      console.log(`  Post ${result.postId}: mediaItems=${post.mediaItems?.length}, status=${post.platforms?.[0]?.status}`);
      
      # Additional verification: check that it's not animated (we already did, but double-check via content)
      const content = post.content || "";
      if(isCartoonVideoSync(content, "")){
        console.log(`  WARNING: Post ${result.postId} appears animated!`);
      }else{
        console.log(`  Post ${result.postId} passed cartoon check.`);
      }
      
      # Check that mediaItems is a video
      const mediaItems = post.mediaItems || [];
      const videoItem = mediaItems.find(item => item.type === "video");
      if(videoItem){
        console.log(`  Post ${result.postId} has video mediaItem.`);
      }else{
        console.log(`  WARNING: Post ${result.postId} does not have a video mediaItem!`);
      }
    }catch(e){
      console.log(`  Error verifying post ${result.postId}: ${e.message}`);
    }
  }
  
  fs.writeFileSync("/tmp/euro2004_5_results.json", JSON.stringify(results,null,2));
  console.log("\n=== EURO 2004 5 DONE ===");
