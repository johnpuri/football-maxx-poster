...
function getYtDurationSeconds(ytId){
  try{
    execSync(`yt-dlp --dump-json --no-playlist "https://www.youtube.com/watch?v=${ytId}" --no-warnings 2>/dev/null > /tmp/dump_${ytId}.json`,{timeout:15000});
    const j=JSON.parse(fs.readFileSync(`/tmp/dump_${ytId}.json`,"utf8"));
    return j.duration || null;
  }catch{ return null;}
}
function downloadFullNoTrim(url,out){
  // download full video without trim, only later add bar HIGH UP via watermark
...
async function main(){
  ...
  while(results.length<10 && attempts<80){
    ...
    // search 10 candidates to have enough native 120-200 candidates
    const cands=tryYtSearchN(pick.query,10); console.log(`  cands ${cands.length}`);
    let found=false;
    for(let ci=0;ci<cands.length;ci++){
      ...
      // NATIVE DURATION FILTER: 120-200s, skip long 500s videos
      const nativeDur=getYtDurationSeconds(c.id);
      console.log(`    native duration ${nativeDur}s`);
      if(nativeDur===null){console.log("    skip unknown duration");continue;}
      if(nativeDur<120 || nativeDur>200){console.log(`    SKIP duration ${nativeDur}s not in 120-200 (prefer native 2-3 min, not trimming long videos)`);continue;}
      // only now parse and validate
      ...
      const highlight={id:`native-${c.id}`,title:`${parsed.tournament} ${parsed.year} — ${parsed.homeTeam} vs ${parsed.awayTeam}`,league:`${parsed.tournament} ${parsed.year}`,homeTeam:parsed.homeTeam,awayTeam:parsed.awayTeam,tournament:parsed.tournament,year:parsed.year,date:`${parsed.year}-07-01`,stage:parsed.stage,ytTitle:c.title,candidateTitle:c.title,videoUrl:`https://www.youtube.com/watch?v=${c.id}`,embedUrl:`https://www.youtube.com/watch?v=${c.id}`,thumbnail:c.thumbnail,source:"native-aligned",ytId:c.id};
      ...
      const raw=`/tmp/raw_native_${results.length}_${ci}.mp4`;
      try{downloadFullNoTrim(highlight.videoUrl,raw);}catch(e){console.log(`    download fail ${e.message.slice(0,120)}`);continue;}
      const dur=getDuration(raw); console.log(`    file duration ${dur?.toFixed(1)}s (must be 120-200 native)`);
      if(dur===null||dur<118||dur>205){console.log(`    skip file duration ${dur} not native 2-3`);try{fs.unlinkSync(raw);}catch{};continue;}
      ...
      // watermark HIGH UP single bar — no trim, full video + pad 110px
      const watermarked=`/tmp/wm_native_${results.length}.mp4`;
      console.log(`    WATERMARK HIGH UP 110 left logo ${logoPath} — NO TRIM, full ${Math.round(dur)}s`);
      ...
      results.push({highlight,content,ytTitle:c.title,ytUrl:highlight.videoUrl,publicUrl,postId,duration:Math.round(dur),nativeDur,wmDur,logo:logoPath,bar:"HIGH UP pad 110px left",verified:false});
      seen.add(key); try{fs.unlinkSync(raw);}catch{}; found=true; break;
    }
    if(!found) console.log(`  No valid NATIVE 120-200 candidate for ${pick.query} — trying next pick`);
  }
  console.log("\n=== VERIFY ===");
  for(const r of results){
    try{
      ...
      console.log(`${r.postId} ${r.highlight.tournament} ${r.highlight.year} ${r.highlight.homeTeam} vs ${r.highlight.awayTeam} native=${r.nativeDur}s file=${r.duration}s media=${mi.length} type=${mi[0]?.type} verified=${r.verified}`);
    }catch(e){console.log(`verify err ${r.postId} ${e.message}`);}
  }
  fs.writeFileSync("/tmp/post_native_results.json",JSON.stringify(results,null,2));
  console.log(`\n=== DONE ${results.length}/10 NATIVE 120-200 NO TRIM ===`);
  console.log(JSON.stringify(results.map(r=>({postId:r.postId,tournament:r.highlight.tournament,year:r.highlight.year,teams:`${r.highlight.homeTeam} vs ${r.highlight.awayTeam}`,ytTitle:r.ytTitle,nativeDur:r.nativeDur,verified:r.verified})),null,2));
}
