...
function downloadWithInfo(url,out){let info=null;try{const j=execSync(`yt-dlp --dump-json --no-playlist "${url}" 2>/dev/null | head -n 1`,{encoding:"utf8",timeout:15000}).trim();if(j) info=JSON.parse(j);}catch{} const cmd=`yt-dlp --no-playlist --max-downloads 1 -f "bv*[height<=720][ext=mp4]+ba[ext=m4a]/b[height<=720]/best" --merge-output-format mp4 -o "${out}" "${url}"`; console.log(cmd); try{execSync(cmd,{stdio:"inherit",timeout:180000});}catch(e){if(!fs.existsSync(out)) throw e;} if(!fs.existsSync(out)) throw new Error(`download failed ${out}`); return info;}
...
function tryYtSearch10(query){try{const out=execSync(`yt-dlp "ytsearch10:${query}" --get-id --get-title --no-warnings 2>/dev/null | head -n 20`,{timeout:30000,encoding:"utf8"}).trim();const lines=out.split("\n").filter(Boolean);const cands=[];for(let i=0;i<lines.length-1;i+=2){const title=lines[i];const id=lines[i+1];if(/^[A-Za-z0-9_-]{6,}$/.test(id)) cands.push({id,title,thumbnail:`https://img.youtube.com/vi/${id}/hqdefault.jpg`});} if(!cands.length){const ids=out.split("\n").map(s=>s.trim()).filter(s=>/^[A-Za-z0-9_-]{6,}$/.test(s));for(const id of ids) cands.push({id,title:"",thumbnail:`https://img.youtube.com/vi/${id}/hqdefault.jpg`});} return cands;}catch{return [];}}
function tryYtSearch5(query){return tryYtSearch10(query);}

// Unpublish double-title / mismatched
async function unpublishBad(){
  console.log("=== UNPUBLISH CHECK (double-title / mismatched) ===");
  try{
    const res=await fetch(`${config.zernioBaseUrl}/posts?limit=50`,{headers:{Authorization:`Bearer ${config.zernioApiKey}`}});
    const t=await res.text(); let posts; try{posts=JSON.parse(t);}catch{posts=[];} if(!Array.isArray(posts)) posts=posts.data||posts.posts||[posts];
    console.log(`Found ${posts.length} posts`);
    let count=0;
    for(const post of posts){
      if(!post) continue; const postId=post._id||post.id; if(!postId) continue;
      const content=post.content||""; const mediaItems=post.mediaItems||[];
      const isFB = post.platforms?.some(p=>p.platform==="facebook");
      if(!isFB) continue;
      let bad=false, reason="";
      // double title: tournament year appears twice or content first line duplicated
      const lines=content.split("\n").filter(Boolean);
      if(/youtube\.com|youtu\.be/i.test(content)){bad=true;reason="youtube link";}
      // double-title heuristic: same team appears 4+ times or tournament duplicated like "Euro 2004.*Euro 2004"
      if(!bad && /(Euro|World Cup|Premier League|Champions League|La Liga|Serie A|Bundesliga|Ligue 1).*\1/i.test(content)){bad=true;reason="double-title tournament dup";}
      if(!bad && isCartoonVideoSync(content,"")){bad=true;reason="cartoon keyword";}
      // skip unpublish for published video posts that look ok - only unpublish bad
      if(bad){
        console.log(`Unpublishing ${postId} reason=${reason} content=${content.slice(0,80)}`);
        try{
          const r=await fetch(`${config.zernioBaseUrl}/posts/${postId}/unpublish`,{method:"POST",headers:{Authorization:`Bearer ${config.zernioApiKey}`,"Content-Type":"application/json"},body:JSON.stringify({platform:"facebook"})});
          const txt=await r.text(); console.log(` -> ${r.status} ${txt.slice(0,200)}`); count++;
        }catch(e){console.log(`unpublish err ${e.message}`);}
      }
    }
    console.log(`Unpublish done, unpublished ${count}`);
  }catch(e){console.error("unpublish check error",e.message);}
}

async function main(){
  await unpublishBad();
  console.log("\n=== POSTING 10 REELS DIVERSE ===");
  ...
  // force Euro 2004 first pick
  const euroPick={tournament:"Euro",year:2004,match:{homeTeam:"Greece",awayTeam:"Portugal",title:"Euro 2004 Final — Greece vs Portugal",city:"Lisbon"},title:"Euro 2004 Final — Greece vs Portugal",query:"Euro 2004 Final Greece vs Portugal highlights",category:"country"};
  let queue=[euroPick];
  while(results.length<10 && attempts<80){
    attempts++;
    if(queue.length===0){
      const batch=getDiverseBatch(8);
      for(const p of batch){
        const key=`${p.tournament}-${p.year}-${p.match.homeTeam}-${p.match.awayTeam}`;
        if(seen.has(key)) continue;
        if(/world cup/i.test(p.tournament) && /final/i.test(p.title)) continue;
        // avoid duplicate Euro 2004
        if(p.tournament==="Euro" && p.year===2004 && results.some(r=>r.highlight.year===2004 && r.highlight.tournament==="Euro")) continue;
        queue.push(p);
      }
    }
    ...
    if(seen.has(key)) continue;
    console.log(`\n--- pick ${results.length+1}/10: ${pick.tournament} ${pick.year} ${pick.match.homeTeam} vs ${pick.match.awayTeam} query=${pick.query} ---`);
    ...
    for(let ci=0;ci<cands.length;ci++){
      ...
      const highlight={id:`final-${c.id}`,title:`${parsed.tournament} ${parsed.year} — ${parsed.homeTeam} vs ${parsed.awayTeam}`,league:`${parsed.tournament} ${parsed.year}`,homeTeam:parsed.homeTeam,awayTeam:parsed.awayTeam,tournament:parsed.tournament,year:parsed.year,date:`${parsed.year}-07-01`,stage:parsed.stage,ytTitle:c.title,candidateTitle:c.title,videoUrl:`https://www.youtube.com/watch?v=${c.id}`,embedUrl:`https://www.youtube.com/watch?v=${c.id}`,thumbnail:c.thumbnail,source:"final-aligned",ytId:c.id};
      ...
      // NATIVE DURATION FILTER: 120-200s only, prefer not to trim — skip 500s videos
      const nativeDur=getYtDurationSeconds(c.id); console.log(`    native duration ${nativeDur}s`); if(nativeDur===null||nativeDur<120||nativeDur>210){console.log(`    SKIP native duration ${nativeDur} not in 120-210 — picking next candidate (2-3 min native, no trim)`);continue;}
      const raw=`/tmp/raw_final_${results.length}_${ci}.mp4`; let info=null; try{info=downloadWithInfo(highlight.videoUrl,raw);}catch(e){console.log(`    download fail ${e.message.slice(0,120)}`);continue;}
      const dur=getDuration(raw); console.log(`    file duration ${dur?.toFixed(1)}s (must be 120-210 native, no trim)`); if(dur===null||dur<118||dur>215){console.log(`    skip file duration ${dur} not native 2-3 min`);try{fs.unlinkSync(raw);}catch{};continue;}
      ...
      // watermark HIGH UP single bar skipTitleBar false
      const watermarked=`/tmp/wm_final_${results.length}.mp4`;
      ...
      results.push({highlight,content,ytTitle:c.title,ytUrl:highlight.videoUrl,publicUrl,postId,duration:Math.round(dur),watermarkedDuration:wmDur,logo:logoPath,bar:"HIGH UP pad 110px left",watermark:"top-right 140 0.6 covering official",verified:false});
  ...
  for(const r of results){
    try{
      ...
      const okMedia=mi.length===1&&mi[0].type==="video";
      const okPub=plat?.status==="published"||plat?.status==="publishing";
      const content=p.content||r.content;
      const teamsOk=r.highlight.homeTeam.toLowerCase().split(" ")[0] && content.toLowerCase().includes(r.highlight.homeTeam.toLowerCase().split(" ")[0]) && content.toLowerCase().includes(r.highlight.awayTeam.toLowerCase().split(" ")[0]);
  ...
  console.log(JSON.stringify(results.map(r=>({postId:r.postId,tournament:r.highlight.tournament,year:r.highlight.year,teams:`${r.highlight.homeTeam} vs ${r.highlight.awayTeam}`,ytTitle:r.ytTitle,duration:r.duration,logo:r.logo,verified:r.verified})),null,2));
  if(results.length<10) console.log(`WARNING only ${results.length}/10`);
  else console.log("SUCCESS 10 posted");
}
